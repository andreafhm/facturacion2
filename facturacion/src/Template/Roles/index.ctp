    ...
    <div class="col-md-12">
        ...
        <div class='page-header'>
            <h3>
                ...
                <?= $this->Html->link($this->Html->tag('i', '', ['class' => 'fa fa-plus']).__('New'), ['controller' => 'Roles', 'action' => 'add'], ['class' => 
           ...
           <table class='table'>
                ...
                <tbody>
                   <?php foreach ($roles as $roles): ?>
                    <tr>
                        <td><?= $this->Number->format($roles->id) ?></td>
                        <td><?= h($roles->name) ?></td>
                        <td><?= $this->Number->format($roles->status) ?></td>
                        <td><?= h($roles->created) ?></td>
                        <td><?= h($roles->modified) ?></td>
                        <td class="actions">
                            <?= $this->Html->link(__('View'), ['action' => 'view', $roles->id]) ?>
                            <?= $this->Html->link(__('Edit'), ['action' => 'edit', $roles->id]) ?>
                            <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $roles->id], ['confirm' => __('Are you sure you want to delete # {0}?', $roles->id)]) ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
